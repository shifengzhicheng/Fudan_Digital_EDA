...
int findregister(vector<pair<string,int>> REGi,string val) {
...
vector<std::pair<node,int>> bindcomputeresource(DataFlowGraph& DFG, vector<pair<string, int>>REGi, vector<computeresource> &CORE) {
	vector<pair<node, int>> ans;
	//从头到尾遍历oplist，除去头部的空op和尾部的空op不用绑定计算资源
	for (vector<node>::iterator it = DFG.get_opList().begin()+1; it != DFG.get_opList().end()-1;it++ ) {
		...
		for (int i = 0; i < CORE.size();i++) {
			if (CORE[i].outputregister == reg && resourcetype == CORE[i].flag) {//找到了输出寄存器匹配的对应计算资源
				...
				for (vector<pair<node, int>>::iterator it2 = ans.begin(); it2 != ans.end(); it2++) {
					if (it2->second == i) {//说明该DFG之前有匹配过该计算资源
						if (it2->first.getTstart() > it->getTend() || it2->first.getTend() < it->getTstart()) {//满足时序
							continue;
						}
				...
				if (flagtimejudgement == 0) {//匹配到之前已经实例化的计算资源
					...
					for (vector<string>::iterator it3 = it->InputVar.begin(); it3 != it->InputVar.end(); it3++) {
		...
		if (flag1 == 0&&resourcetype!=0) {//未匹配到实例化的计算资源,则实例化一个新的计算资源，并将该资源匹配给该node
			...
			ans.push_back(make_pair((*it), CORE.size()));
			CORE.push_back(com);
