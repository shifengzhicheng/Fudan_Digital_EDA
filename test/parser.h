...
public:
	...
	int get_type() {return _type;}  // 获取语句类型
	void set_type(int type) {_type = type;}  // 设置语句类型
	int get_num_oprands() {return _num_oprands;}  // 获取操作数数量
	void set_num_oprands(int t) {_num_oprands = t;}  // 设置操作数数量
	std::string & get_oprand(int num) {return _oprands[num];}  // 获取操作数
	void add_oprand(const std::string & oprand) {_oprands.push_back(oprand);}  // 添加操作数
	void set_var(const std::string & var) {_var = var;}  // 设置变量名称
	const std::string get_var() {return _var;}  // 获取变量名称
...
public:
	...
	void set_name(const std::string & n) {_label = n;}  // 设置基本块名称
	void add_statement(statement & s);  // 添加语句
	void clear_statements();  // 清除语句
	std::vector<statement> & get_statements() {return _statements;}  // 获取语句向量
	std::string get_label_name() {return _label;}  // 获取标签名称
