...
public:
		...
		for (int i = 1; i < List.size() - 1; i++) {
			...
			if (List[i].element.getOPtype() == OP_BR) {
				for (int m = 0; m < DFG.get_outputList().size(); m++) {
					...
					cycle[total_cycle + 1].Statements.push_back(temp);
				}
			}
	...
	void PrintCycle() {
		std::cout << "/********************\t" << DFG.get_label() << "\t*******************/\n" << std::endl;
		for (int i = 0; i < C.size(); i++) {
			std::cout << "Cycle: " << i << "\tExecute:" << std::endl;

			for (int j = 0; j < C[i].Statements.size(); j++) {
				std::cout << "VarNames:";
				for (int k = 0; k < C[i].Statements[j].vars.size(); k++) {
					std::cout << C[i].Statements[j].vars[k] << "\t";
				}
				std::cout << std::endl;
				std::cout << "Binding Registers:";
				for (int k = 0; k < C[i].Statements[j].regs.size(); k++) {
					std::cout << C[i].Statements[j].regs[k] << "\t";
				}
				std::cout << std::endl;
				std::cout << "OPtype:" << C[i].Statements[j].optype << std::endl;
				std::cout << "Computeresource Index:" << C[i].Statements[j].compute_resource_index << std::endl;
				std::cout << "Output Register:" << C[i].Statements[j].outreg << std::endl;
				std::cout << "\n";
			}
		}
		std::cout << "\n\n";
