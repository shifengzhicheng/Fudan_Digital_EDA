...
std::vector<std::vector<int>>creatematrix(DataFlowGraph& DFG,std::vector<int>list, std::vector<std::pair<std::string, int>>REGi, std::vector<computeresource>& CORE,int flag,Hardware&hardware) {//flag表征生成的矩阵计算资源类别
	...
	for (std::vector<int>::iterator it = list.begin(); it != list.end(); it++) {//对应矩阵行
		std::vector<int>ans1;
		if (flag == 1) {
			for (int i = 0; i < row; i++) {
				int val = 0;
...
int maxcompair(std::vector<std::vector<cost_matrix_node>>&matrix2) {
	...
	for (int i = 0; i < zeros.size(); i++) {
		if (matrix2[zeros[i].col][zeros[i].row].flag == 0) {//说明尚未被覆盖
			int ll = 0;
			...
			if (ll == 0) {//说明该0并没有被某一个圈包括，需要自己生成一个圈
...
//匈牙利算法化简
std::vector<std::pair<int, int>> Hungarian(DataFlowGraph& DFG, std::vector<int>&list,std::vector<std::pair<std::string, int>>REGi, std::vector<computeresource>& CORE, std::vector<std::vector<int>>matrix, int flag, Hardware& hardware,int&k) {//k代表已经绑定的op个数
	std::vector<std::pair<int, int>>ans;
	int num = -1;
	if (list.size() == 0) {//如果list为空，则不匹配
		return ans;
	}
	//匈牙利算法第一步，完成
	...
	while (pair < pairsum) {
		...
		for (int i = 0; i < matrix2.size(); i++) {
			for (int j = 0; j < matrix2[0].size(); j++) {
				if (matrix2[i][j].flag == 0) {//从未覆盖的元素中找最小值
	...
	for (std::vector<circle>::iterator it = cir.begin(); it != cir.end(); it++) {//每一个圈都要进行一个计算资源匹配
