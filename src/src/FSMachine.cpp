...
void FSMachine::FSMgener(ControlFlowGraph &CFG)
{
    ...
    std::string FSM_reg_cur_state(std::string("\treg [" + std::to_string(FSMsize - 1) + ":0] " + CurrentState + ";"));
    std::string FSM_reg_fore_state(std::string("\treg [" + std::to_string(FSMsize - 1) + ":0] " + LastState + ";"));
    ...
    for (int i = 0; i < FSMsize; i++)
    {
        ...
        for (int i = 0; i < CurNode.controlnodes.size(); i++)
        {
            ...
            if (CurEdge.cond == UnConditonal)
            {
                ...
                {
                    cond = std::string(
                        CurrentState + " == " + "state_" + CurEdge.From_Block + " & " + Branch_ready + " == " + "1'b1");
            ...
            {
                cond = std::string(
                    CurrentState + " == " + "state_" + CurEdge.From_Block + " & " + Branch_ready + " == " + "1'b1 & cond == 1'b");
                if (CurEdge.cond == IfTrue)
                    cond.append("1");
                else
                    cond.append("0");
                getFSM().push_back("\t" + elseiflogic(cond));
